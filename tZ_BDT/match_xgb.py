...
gbm = xgb.cv(params, xgb_train, num_boost_round=600, verbose_eval=True)
...
pickle.dump(bst, open('models/xgb_2j.dat', "wb"), protocol=2)
...
plt.savefig('plots_2j/test_score.png')
...
plt.ylabel('NEvents')
plt.legend(loc='upper right')
plt.savefig('plots_2j/train_score.png')

minLen = min([len(testPredTrue), len(testPredFalse)])
plt.figure()
plt.hist(testPredTrue[:minLen], 30, range=(-0.1,1.1), log=False, alpha=0.5, label='WZ - Test')
plt.hist(testPredFalse[:minLen], 30, range=(-0.1,1.1), log=False, alpha=0.5, label='tZ - Test')
plt.hist(trainPredTrue[:minLen], 30, range=(-0.1,1.1), log=False, histtype='step', alpha=0.5, label='WZ - Train')
plt.hist(trainPredFalse[:minLen], 30, range=(-0.1,1.1), log=False, histtype='step', alpha=0.5, label='tZ - Train')
plt.title("BDT Output, max depth=%i" %(maxDepth))
plt.xlabel('BDT Score')
plt.ylabel('NEvents')
plt.legend(loc='upper right')
plt.savefig('plots_2j/xgb_score.png')

plt.figure()
fip = xgb.plot_importance(bst)
plt.title("xgboost feature important")
plt.legend(loc='lower right')
plt.savefig('plots_2j/feature_importance.png')
